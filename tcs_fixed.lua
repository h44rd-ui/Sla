...
local function resolveTPS()
    -- Fonte principal: a TPS oficial dentro de WorkspaceLeaderboards.
    local folder = Workspace:FindFirstChild(TPS_FOLDER_HINT)
    if folder then
        local t = folder:FindFirstChild("TPS")
        if t and (t:IsA("BasePart") or t:IsA("Model")) then
            return t
        end
    end

    -- Fallback apenas para TPS diretamente dentro de Workspace.
    -- Nunca procura TPS em todos os descendants, evitando pegar outra bola.
    local direct = Workspace:FindFirstChild("TPS")
...
local function scheduleTPSTry()
    defer(function()
        local resolved = resolveTPS()

        -- Não troca de bola só porque apareceu outra TPS.
        if resolved then
            if not _tpsCache or not _tpsCache.Parent then
                _tpsCache = resolved
                CurrentTPS = resolved
            end
        elseif _tpsCache and not _tpsCache.Parent then
            _tpsCache = nil
            CurrentTPS = nil
        end
    end)
...
local function FindTPS() return _tpsCache end
...
local function UpdateESP()
    ...
    local ballPart = GetBallPart(CurrentTPS)
    local Position = ballPart and ballPart.Position or GetPosition(CurrentTPS)

    if ESPBall and ballPart then
        -- Highlight fica preso à mesma peça da bola.
        ESPHighlight.Adornee = ballPart
    ...
    if not ESPPrediction or not ballPart or not Position or not ESPLine then
        if ESPLine then ESPLine.Visible = false end
        return
    end
...
local AIMBOT_GOALS = {
    ["Goal 1"] = {
        Vector3.new(-20, -29, -204),
        Vector3.new(-33, -29, -204),
    },
    ["Goal 2"] = {
        Vector3.new(-17, -29, 389),
        Vector3.new(-2, -29, 388),
    },
}
...
local function getNearestAimbotGoal(position)
    ...
    for goalName, points in pairs(AIMBOT_GOALS) do
        local goalDist = math.huge
        for i = 1, #points do
            local d = (points[i] - position).Magnitude
            if d < goalDist then goalDist = d end
        end

        if goalDist < bestDist then
            bestDist = goalDist
            bestName = goalName
        end
    end

    if not bestName then return nil, nil, math.huge end

    local side = GoalSide[bestName] or 1
    local target = AIMBOT_GOALS[bestName][side]
    return bestName, target, bestDist
end

local function toggleGoalSide(goalName)
    if not goalName or not AIMBOT_GOALS[goalName] then return end
    GoalSide[goalName] = ((GoalSide[goalName] or 1) == 1) and 2 or 1
end
...
local function resetAimbotState()
    ...
    AimbotGoal.TouchBall        = nil
    AimbotGoal.ControlBall      = nil
    AimbotGoal.ActiveGoalName   = nil
    AimbotGoal.ActiveGoal       = nil
end
...
local function bindAimbotTouch()
    ...
    local ball = GetBallPart(_tpsCache)
    if not ball then return end

    AimbotGoal.TouchBall = ball

    AimbotGoal.TouchConn = ball.Touched:Connect(function()
        if not AimbotGoal.Enabled then return end
        if AimbotGoal.TouchBall ~= ball then return end
        ...
        if (ball.Position - root.Position).Magnitude > AimbotGoal.DistanceToBall then return end
        ...
        AimbotGoal.ActiveGoalName   = nil
        AimbotGoal.ActiveGoal       = nil
        AimbotGoal.LastBallVelocity = ball.AssemblyLinearVelocity
    end)
end

local function detectAimbotShot(ball)
    if not ball or AimbotGoal.TouchBall ~= ball then return false end
    if AimbotGoal.LastTouchTime <= 0 then return false end

    if tick() - AimbotGoal.LastTouchTime > 1 then
        AimbotGoal.LastTouchTime = 0
        return false
    end
...
local function setTelekinesis(v)
    ...
    if v then
        ...
        -- Captura UMA bola. Não procura outra a cada frame.
        local ball = GetBallPart(_tpsCache)
        Telekinesis.Ball = ball
...
RunService.Heartbeat:Connect(function()
    ...
    if AimbotGoal.Enabled then
        local ball = AimbotGoal.ControlBall or AimbotGoal.TouchBall

        -- O alvo fica travado na bola que realmente recebeu o toque.
        if ball and ball.Parent then
            if not AimbotGoal.BallControl and not AimbotGoal.ShotDetected then
                if detectAimbotShot(ball) then
                    ...
                    AimbotGoal.ControlBall = ball

                    local goalName, goalPoint = getNearestAimbotGoal(ball.Position)
                    AimbotGoal.ActiveGoalName = goalName
                    AimbotGoal.ActiveGoal = goalPoint
                end
            end

            if AimbotGoal.BallControl and AimbotGoal.ControlBall == ball then
                local goal = AimbotGoal.ActiveGoal
                ...
                if goal then
                    ...
                    if dist <= AimbotGoal.DistanceToGoal then
                        ...
                        -- Próximo chute usa o outro lado do mesmo gol.
                        toggleGoalSide(AimbotGoal.ActiveGoalName)
    ...
    if Telekinesis.Enabled then
        ...
        if ball and ball.Parent and camera then
            -- Mantém a câmera normal, apenas usando sua direção atual.
            ...
            ball.AssemblyLinearVelocity =
                ball.AssemblyLinearVelocity:Lerp(target, Telekinesis.Smoothness)
        elseif not ball or not ball.Parent then
            Telekinesis.Enabled = false
            Telekinesis.Ball = nil
        end
    end
...
RunService.RenderStepped:Connect(function()
    -- Só troca a referência quando a bola alvo realmente deixou de existir.
...
Workspace.DescendantRemoving:Connect(function(d)
    ...
    if d == CurrentTPS or d == _tpsCache then
        ...
        if AimbotGoal.TouchBall == d or AimbotGoal.ControlBall == d or GetBallPart(d) then
            resetAimbotState()
            if AimbotGoal.TouchConn then
                AimbotGoal.TouchConn:Disconnect()
                AimbotGoal.TouchConn = nil
        ...
        if Telekinesis.Ball == d or GetBallPart(d) then
            Telekinesis.Ball = nil
            Telekinesis.Enabled = false
        end
...
Workspace.DescendantAdded:Connect(function(d)
    ...
    -- Não reescaneia qualquer TPS aleatório. Só tenta resolver quando
    -- o alvo preferencial realmente apareceu.

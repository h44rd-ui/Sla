...
local _ballTouchConn = nil
local _touchBoundBall = nil
local _refreshingBall = false
local RefreshBall

-- The game's REAL ball is always this exact object/path.
-- Never search for another TPS anywhere else in Workspace.
...
-- Always reads the CURRENT TPS and CURRENT physical part.
-- This function deliberately does not trust an old cached ball.
local function ReadLiveBall()
    local tps = FindTPS()
    local part = GetBallPart(tps)
    return tps, part
end

local function resetBallDependentState()
    if AimbotGoal then
...
local function bindLiveBallTouch(ball)
    if _touchBoundBall == ball and _ballTouchConn then return end

    if _ballTouchConn then
        _ballTouchConn:Disconnect()
        _ballTouchConn = nil
    end
    _touchBoundBall = ball

    if not ball then return end

    _ballTouchConn = ball.Touched:Connect(function()
        -- A touch is also a refresh signal: the game can replace/rebuild TPS
        -- immediately after a player touches the ball.
        RefreshBall(true)
    end)
end

-- Refreshes the ORIGINAL game ball on every call.
-- It is called by every ball-dependent feature and every frame/heartbeat.
RefreshBall = function(force)
    if _refreshingBall then
        return CurrentTPS, CurrentBall, _ballGeneration
    end

    _refreshingBall = true

    local tps, part = ReadLiveBall()
    local changed = (tps ~= CurrentTPS or part ~= CurrentBall)
    ...
    bindLiveBallTouch(part)

    if changed then
        resetBallDependentState()
    end

    _refreshingBall = false
    return CurrentTPS, CurrentBall, _ballGeneration
end
...
local function getNearestBall()
    local _, ball = RefreshBall()
    local hrp = State.HRP
    if not ball or not hrp then return nil, math.huge end
...
local function UpdateESP()
    ...
    if not ESPPrediction or not ESPLine then
        if ESPLine then ESPLine.Visible = false end
        return
    end

    local predictedPosition = position + ball.AssemblyLinearVelocity * PredictionTime
    ...
    if not startVisible or not endVisible then
        ESPLine.Visible = false
        return
    end
...
local Goal1Points     = { Vector3.new(-20, -29, -204), Vector3.new(-33, -29, -204) }
local Goal2Points     = { Vector3.new(-17, -29, 389), Vector3.new(-2, -29, 388) }
local GoalSides       = { ["Goal 1"] = 1, ["Goal 2"] = 1 }
local _goalGen        = 0
...
local AimbotGoal = {
    ...
    TouchConn = nil, ConnectedBall = nil, MobileRefs = nil,
    DistanceToGoal = 3, DistanceToBall = 8,
    ControlledBall = nil, ControlledTPS = nil,
}

local Telekinesis = {
    Enabled = false, Speed = 180, Smoothness = 0.25,
    OldCameraSubject = nil, MobileRefs = nil,
    ControlledBall = nil,
}

local function getGoalPoints(goalName)
    return goalName == "Goal 2" and Goal2Points or Goal1Points
end

local function getNearestAimbotGoal(position)
    local goalName, bestDist = nil, math.huge
    for _, name in ipairs({ "Goal 1", "Goal 2" }) do
        local points = getGoalPoints(name)
        for i = 1, #points do
            local d = (points[i] - position).Magnitude
            if d < bestDist then bestDist, goalName = d, name end
        end
    end
    if not goalName then return nil end
    local points = getGoalPoints(goalName)
    local point = points[GoalSides[goalName] or 1]
    return point, bestDist, goalName
end
...
local function bindAimbotTouch()
    ...
    if ball == AimbotGoal.ConnectedBall and AimbotGoal.TouchConn then return end

    if AimbotGoal.TouchConn then
        AimbotGoal.TouchConn:Disconnect()
        AimbotGoal.TouchConn = nil
    end
    AimbotGoal.ConnectedBall = ball
    resetAimbotState()
    if not ball then return end

    local boundBall = ball
    AimbotGoal.TouchConn = boundBall.Touched:Connect(function()
        if not AimbotGoal.Enabled or GetCurrentBall() ~= boundBall then return end
        if not hasShootTool() then return end
        local char = LocalPlayer.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
        if not root or (boundBall.Position - root.Position).Magnitude > AimbotGoal.DistanceToBall then return end
...
local function detectAimbotShot(ball)
    if not ball or GetCurrentBall() ~= ball then return false end
    if AimbotGoal.LastTouchTime <= 0 or tick() - AimbotGoal.LastTouchTime > 1 then return false end
...
local function setTelekinesis(v)
    local camera = Workspace.CurrentCamera
    if not camera then Telekinesis.Enabled = false; return end
    if v then
        if not Telekinesis.Enabled then Telekinesis.OldCameraSubject = camera.CameraSubject end
        Telekinesis.Enabled = true
        local ball = GetCurrentBall()
        Telekinesis.ControlledBall = ball
        if ball then camera.CameraSubject = ball end
    else
        local ball = GetCurrentBall()
        if ball and ball.Parent then ball.AssemblyLinearVelocity = Vector3.zero end
        Telekinesis.Enabled = false
        Telekinesis.ControlledBall = nil
    ...
    if Telekinesis.MobileRefs then applyButtonState(Telekinesis.MobileRefs, Telekinesis.Enabled, "TELECINESE\nON", "TELECINESE\nOFF") end
end
...
local function RunAutoGoalGen(gen)
    while AutoGoal and _goalGen == gen do
        ...
        local Ball = GetCurrentBall()
        if not Root or not Ball then task.wait(0.01); continue end

        local GoalPosition = (SelectedGoal == "Goal 2") and Goal2Points[GoalSides["Goal 2"] or 1] or Goal1Points[GoalSides["Goal 1"] or 1]
        local DirectionVector = GoalPosition - Ball.Position
        local Distance = DirectionVector.Magnitude
        if Distance <= 2 then
            Shoot()
            GoalSides[SelectedGoal] = (GoalSides[SelectedGoal] == 1) and 2 or 1
            task.wait(ShootCooldown)
            continue
        end

        local Direction = DirectionVector.Unit
        Root.CFrame = CFrame.lookAt(Ball.Position - Direction * 0.8 + Vector3.new(0, 0.3, 0), Ball.Position)
        task.wait(0.015)
        Ball = GetCurrentBall()
        if not Ball then continue end
        DirectionVector = GoalPosition - Ball.Position
        ...
        Direction = DirectionVector.Unit
        Root.CFrame = CFrame.lookAt(Ball.Position + Direction * math.min(2.5, Distance) + Vector3.new(0, 0.3, 0), GoalPosition)
        Shoot()
        task.wait(ShootCooldown)
    end
end
...
local function findTarget(part)
    local ball = GetCurrentBall()
    if not ball or not part then return nil end
    local c = part
    while c and c ~= Workspace do
        if c == ball then return ball end
        c = c.Parent
    end
    return nil
end
...
RunService.Heartbeat:Connect(function()
    -- HARD REFRESH: the game may replace TPS when ANY player touches the ball.
    RefreshBall(true)
    local now = clock()
    Reach:step(now, "")
    GKReach:step(now, "GK:")

    local liveBall = GetCurrentBall()
    if AimbotGoal.Enabled then
        bindAimbotTouch()
        if liveBall then
            if not AimbotGoal.BallControl and not AimbotGoal.ShotDetected then
                if detectAimbotShot(liveBall) then
                    AimbotGoal.ShotDetected = true
                    AimbotGoal.BallControl = true
                    AimbotGoal.ControlledBall = liveBall
                    AimbotGoal.ControlledTPS = GetCurrentTPS()
                end
            end
            if AimbotGoal.BallControl and AimbotGoal.ControlledBall == liveBall then
                local goal = getNearestAimbotGoal(liveBall.Position)
                if goal then
                    local diff = goal - liveBall.Position
                    if diff.Magnitude <= AimbotGoal.DistanceToGoal then
                        liveBall.AssemblyLinearVelocity = Vector3.zero
                        local _, _, goalName = getNearestAimbotGoal(liveBall.Position)
                        if goalName then GoalSides[goalName] = (GoalSides[goalName] == 1) and 2 or 1 end
                        resetAimbotState()
                    else
                        liveBall.AssemblyLinearVelocity = liveBall.AssemblyLinearVelocity:Lerp(diff.Unit * AimbotGoal.Speed, AimbotGoal.Smoothness)
                    end
                end
            end
        else
    ...
    if Telekinesis.Enabled then
        ...
        if camera and ball then
            ...
            local target = camera.CFrame.LookVector * Telekinesis.Speed
...
Workspace.DescendantAdded:Connect(function(d)
    if d.Name == "TPS" or d.Name == "WorkspaceLeaderboards" or (d.Parent and d.Parent.Name == "WorkspaceLeaderboards") then
        defer(function()
            RefreshBall()
            ...
            if Telekinesis.Enabled then
                local ball = GetCurrentBall()
                local camera = Workspace.CurrentCamera
                if ball and camera then camera.CameraSubject = ball end
            end
        end)
    end
end)
...
do
    local function bindLeaderboards(folder)
    ...
    bindLeaderboards(Workspace:FindFirstChild("WorkspaceLeaderboards"))
    Workspace.ChildAdded:Connect(function(child)
        if child.Name == "WorkspaceLeaderboards" then
            bindLeaderboards(child)
            task.defer(function() RefreshBall(true) end)
        end
    end)
end
...
do
    ...
    sec:Toggle({
        ...
        Callback = function(v)
            if v then
                local _, refs = createMobileButton("LuaTelekinesisMobile", "TELECINESE\nOFF", function()
                    setTelekinesis(not Telekinesis.Enabled)
                    onConfigChanged()
                end)
                Telekinesis.MobileRefs = refs
                if Telekinesis.Enabled then applyButtonState(refs, true, "TELECINESE\nON", "TELECINESE\nOFF") end
            else
                setTelekinesis(false)
                destroyMobileButton("LuaTelekinesisMobile")
                Telekinesis.MobileRefs = nil
            end
        end,
    })
end
